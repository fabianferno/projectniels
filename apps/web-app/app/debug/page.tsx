...
export default function Page({ searchParams }: { searchParams: Record<string, string> }): JSX.Element {
    const { farcasterUser, loading, startFarcasterSignerProcess, logout, impersonateUser } = useFarcasterIdentity()
    const router = useRouter()
    const url = searchParams.url
    const [urlInput, setUrlInput] = useState(url || process.env.NEXT_PUBLIC_HOST || "http://localhost:3000")

    const [currentFrame, setCurrentFrame] = useState<ReturnType<typeof getFrame> | undefined>(undefined)
    const [framePerformanceInSeconds, setFramePerformanceInSeconds] = useState<number | null>(null)

    // Load initial frame
    const { data, error, isLoading, mutate } = useSWR<ReturnType<typeof getFrame>>(
    ...
    const submitOption = async ({ buttonIndex, inputText }: { buttonIndex: number; inputText?: string }) => {
        if (!farcasterUser || !farcasterUser.fid || !currentFrame || !currentFrame?.frame || !url) {
            return
        }
        ...
        */
        ...
        const response = await fetch(`/debug/frame-action?${searchParams.toString()}`, {
            method: "POST",
            headers: {
                ...
                <div className="bg-slate-100 mb-4 p-4">
                    <div className="flex flex-row gap-4 items-center">
                        ...
                        >
                            ...
                            />
                            <button className="bg-blue-500 text-white p-2 py-1 rounded-r">Debug</button>
                        </form>
                        ...
                        <button
                            ...
                            onClick={(e) => {
                                e.preventDefault()
                                router.push(`?url=${baseUrl}/examples/niels`)
                            }}
                        >
                            Project Niels
                        </button>
